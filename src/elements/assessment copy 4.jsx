...
class Assessment extends Component {
  ...
  createObj() {
    ...
    for (const len in localRelationsLen) {
      ...
      let synonymsInner = synonyms[0].toLowerCase();
  ...
  searchSymptom = (e) => {
    this.setState(
      {
        symptomQuery: e.target.value.toLowerCase(),
      },
      () => {
    ...
    for (const list in symptoms) {
      for (const match in matchingDiseases) {
        if (
          ...
          this.state.userMatchingDiseases[match].occurance +=
            this.state.diseasesMerge[list].diseaseRelations[
              matchingDiseases[match]
            ];
  ...
  async rank() {
    ...
    db.collection("dummyDiseases")
      ...
      .then((querySnapshot) => {
        ...
        try {
          let definition =
            querySnapshot.data().diseaseDefinitions[0].definition;
